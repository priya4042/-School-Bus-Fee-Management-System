...
const Students: React.FC = () => {
  ...
  const handleDelete = async (id: string, name: string) => {
    const confirmed = await showConfirm(
      'Delete Student?',
      `Delete ${name} from the live student records? A snapshot will still be saved in Reports for deleted students.`,
      'Delete'
    );
    if (confirmed) {
      ...
      closeSwal();
      if (result.success) showToast('Student deleted successfully', 'info');
      else showAlert('Error', result.error || 'Failed to delete student record', 'error');
    }
  };
