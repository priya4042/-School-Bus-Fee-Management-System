  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button 
          onClick={async () => {
            if (window.confirm('Are you sure you want to delete your account? This action is irreversible.')) {
              try {
                await api.delete('/users/delete', { data: { userId: user.id, action: 'delete-user' } } as any);
                logout();
              } catch (error) {
        ...
        <button 
          onClick={logout}
          className="bg-slate-50 text-slate-600 px-8 py-4 rounded-2xl font-bold text-sm border border-slate-100 hover:bg-slate-100 transition-all flex items-center gap-2"
        >
          <LogOut size={18} />
          Sign Out
        </button>
