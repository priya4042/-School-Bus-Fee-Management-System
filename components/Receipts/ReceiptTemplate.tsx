          ...
          <div className="bg-slate-50 rounded-[2rem] p-8 mb-12">
            <div className="space-y-4">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-slate-900 font-bold">₹{Number(receipt.baseFee || 0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 font-medium">Late Fee Penalty</span>
                <span className="text-slate-900 font-bold">₹{Number(receipt.lateFee || 0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 font-medium">Discount Applied</span>
                <span className="text-emerald-600 font-bold">-₹{Number(receipt.discount || 0).toLocaleString()}</span>
              </div>
              <div className="pt-4 border-t border-slate-200 flex justify-between items-center">
                <span className="text-slate-900 font-black uppercase tracking-widest text-xs">Total Amount Paid</span>
                <span className="text-2xl font-black text-primary">₹{Number(receipt.totalPaid || 0).toLocaleString()}</span>
              </div>
            </div>
          </div>
