...
import { Shield, Eye, EyeOff, Lock, Mail, Hash, ArrowLeft, CheckCircle } from 'lucide-react';
        ...
        <AnimatePresence mode="wait">
          {step === 'email' && (
            ...
            >
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 ml-1">Admin Email</label>
              ...
              <button 
                type="submit"
                disabled={isLoading}
                className="w-full bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 py-4 rounded-2xl font-bold shadow-lg hover:opacity-90 transition-all active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2"
              >
                {isLoading ? <div className="w-5 h-5 border-2 border-white/30 dark:border-zinc-900/30 border-t-white dark:border-t-zinc-900 rounded-full animate-spin" /> : "Send OTP"}
              </button>
            </motion.form>
          )}
          ...
          {step === 'otp' && (
            ...
            >
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 outline-none focus:ring-2 focus:ring-zinc-900 dark:focus:ring-white transition-all text-zinc-900 dark:text-white tracking-[0.5em] font-bold text-center"
                    placeholder="000000"
                  />
                </div>
              </div>
              <button 
                type="submit"
                className="w-full bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 py-4 rounded-2xl font-bold shadow-lg hover:opacity-90 transition-all active:scale-[0.98]"
              >
                Verify OTP
              </button>
              <button 
          ...
          {step === 'reset' && (
            ...
            >
              ...
              <button 
                ...
                className="w-full bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 py-4 rounded-2xl font-bold shadow-lg hover:opacity-90 transition-all active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2"
              >
                {isLoading ? <div className="w-5 h-5 border-2 border-white/30 dark:border-zinc-900/30 border-t-white dark:border-t-zinc-900 rounded-full animate-spin" /> : "Reset Password"}
              </button>
            </motion.form>
          )}
        </AnimatePresence>
