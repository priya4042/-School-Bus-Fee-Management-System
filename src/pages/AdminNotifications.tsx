...
const AdminNotifications: React.FC = () => {
  ...
  const handleBroadcast = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    showLoading('Transmitting Broadcast...');
    try {
      ...
      closeSwal();
      showToast(`Broadcast sent to ${data.recipients} users successfully.`, 'success');
      setMessage('');
    } catch (err) {
      console.error(err);
      closeSwal();
      showAlert('Transmission Failed', 'Failed to send broadcast. Please try again.', 'error');
    } finally {
      setLoading(false);
    }
  };
