...
from .barcode_service import generate_fee_barcode
import datetime
    ...
    for student in active_students:
        ...
        if not existing:
            ...
            new_due = models.MonthlyDue(
                ...
                barcode=generate_fee_barcode(student.id, month, year)
            )
            db.add(new_due)
            created_count += 1
...
def generate_dues_for_new_student(db: Session, student_id: int):
    """
    Generate dues for the current month for a newly added student
    """
    now = datetime.datetime.now()
    month = now.month
    year = now.year
    due_date = datetime.datetime(year, month, 10)
